...
- **Script seed chuẩn** (khớp model thật): `npm run seed` — tạo admin + khách + thương hiệu + danh mục + sản phẩm/biến thể + đơn hàng/đánh giá mẫu.
- **Script tạo admin riêng**: `npm run create-admin -- email@example.com MatKhau123 "Tên admin"`.

### Cách vào trang admin
1. `cd server && npm install && npm run dev` (admin sẽ được tạo tự động, xem log).
2. `cd client && npm install && npm run dev`.
3. Đăng nhập `admin@lessencenoire.vn / Admin@123` → bấm **Quản trị** trên Header hoặc vào `/admin`.

### 2. Cấu hình Cloudinary để đồng bộ & quản lý ảnh
Thêm vào `server/.env` rồi khởi động lại server:
```
...
**API mới (đều yêu cầu quyền admin):**
- `GET /api/admin/media/status` — trạng thái cấu hình.
- `GET /api/admin/media?max=&cursor=` — liệt kê ảnh.
- `POST /api/admin/media/upload` (field `images`, tối đa 10) — upload nhiều ảnh.
- `POST /api/admin/media/delete` `{ publicId }` — xoá ảnh.
- `POST /api/admin/upload` (field `image`) — upload 1 ảnh, trả `url` + `publicId`.
