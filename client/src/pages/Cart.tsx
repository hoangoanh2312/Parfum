...
export default function Cart() {
  ...
  async function checkout() {
    ...
    try {
      // Tạo đơn hàng THẬT -> backend sẽ TRỪ tồn kho rồi xóa giỏ
      const { data } = await api.post("/orders", { method: "cod" });
      toast.success(
        "Đặt hàng thành công!\nMã đơn: " +
          data.data.orderId +
          "\nTổng thanh toán: " +
      ...
      toast.error(
        e?.response?.data?.message || "Bạn cần đăng nhập để thanh toán",
      );
      verifyStock();
    }
  }
