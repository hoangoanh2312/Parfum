...
const policySections = [
  {
    ...
    text:
      "Parfum Store tuyển chọn nước hoa chính hãng từ các thương hiệu uy tín, ưu tiên trải nghiệm tư vấn rõ ràng, sản phẩm minh bạch và dịch vụ sau bán hàng chỉn chu.",
  },
  {
    id: "returns",
    ...
    label: "Returns",
    text:
      "Sản phẩm được hỗ trợ đổi trả khi còn nguyên seal, chưa qua sử dụng và có lỗi phát sinh từ vận chuyển hoặc nhà bán hàng. Khách vui lòng liên hệ trong vòng 7 ngày kể từ khi nhận hàng.",
  },
  {
    id: "shipping",
    title: "Vận chuyển",
    label: "Shipping",
    text:
      "Đơn hàng được đóng gói chống sốc và bàn giao cho đơn vị vận chuyển trong thời gian sớm nhất. Phí vận chuyển và thời gian nhận hàng phụ thuộc khu vực giao hàng.",
  },
  {
    id: "warranty",
    title: "Bảo hành",
    label: "Warranty",
    text:
      "Parfum Store hỗ trợ xác minh sản phẩm, kiểm tra tình trạng vòi xịt, nắp chai và các lỗi kỹ thuật liên quan đến bao bì trong quá trình sử dụng ban đầu.",
  },
  {
    id: "contact",
    title: "Liên hệ",
    label: "Contact",
    text:
      "Cần tư vấn mùi hương, hỗ trợ đơn hàng hoặc chính sách sau mua? Liên hệ qua hotline 0328 779 845 hoặc email contact4w@perfume.vn.",
  },
];
...
export default function About() {
  ...
  useEffect(() => {
    ...
    api
      .get<ProductListResponse>("/products", { params: { limit: 10, sort: "newest" } })
      .then(({ data }) => {
  ...
  const rawPaletteItems = useMemo(() => {
    const fromProducts = products
      ...
      .map((product, index) => ({
        ...
        image: product.images?.[0] || product.image || fallbackIngredients[index % fallbackIngredients.length].image,
          ...
          <div>
            ...
            <div className="mt-7 max-w-[570px] space-y-5 text-sm leading-6 text-[#706D66]">
              <p>
                ...
                His method involves “The Void” — a period of sensory deprivation
                before beginning any new composition.
              </p>

              <p>
                With a portfolio spanning three decades, Thorne has redefined
                modern luxury by reintroducing forgotten resins and rare florals
                into the editorial palette.
              </p>
            </div>
          </div>
        </div>
