...
export default function Blog() {
  ...
  const archetypes = useMemo(() => {
    ...
    const items = products
      .filter((product) => product.fragranceFamily && !seen.has(product.fragranceFamily))
      .map((product) => {
        seen.add(product.fragranceFamily || "");

        return {
          name: product.fragranceFamily || product.name,
          image: product.images?.[0] || product.image || fallbackArchetypes[seen.size % fallbackArchetypes.length].image,
        };
      });
  ...
  const articles = useMemo(() => {
    const items = products.slice(0, 5).map((product, index) => ({
      id: index + 1,
      category: product.fragranceFamily || product.category || product.brand || "Scent journal",
      title: product.name,
      description:
        product.description ||
        `A closer look at ${product.name}, its mood, character, and olfactory structure.`,
      image: product.images?.[0] || product.image || fallbackArticles[index % fallbackArticles.length].image,
      slug: product.slug || product.id,
    }));

    return items.length ? items : fallbackArticles.map((article) => ({ ...article, slug: "" }));
  }, [products]);
          ...
          <article className="group">
            ...
            <div className="pt-5">
              ...
              <p className="mt-4 max-w-[720px] text-xs leading-5 text-[#706D66]">
                ...
                travel to the valleys of Bulgaria to witness the fleeting moment
                when the rose&apos;s essential oils are at their zenith.
              </p>
