...
import { BarChart3, Boxes, CircleDollarSign, ClipboardList, Users, Workflow } from "lucide-react";
...
type ReportData = {
  ...
  operations: {
    ...
    support: { total: number; open: number; resolved: number; averageResolutionHours: number | null };
...
export default function AdminReports() {
  ...
  return (
    <div>
      <PageHeader title="Báo cáo kinh doanh" subtitle="Số liệu thực tế từ đơn hàng, thanh toán, tồn kho và yêu cầu hỗ trợ" />
      ...
      {loading || !data ? <LoadingState /> : (
        <div className="space-y-5">
          {tab === "revenue" && <>
            <div className="grid gap-x-6 sm:grid-cols-2 lg:grid-cols-4"><Kpi label="Doanh thu thực nhận" value={formatVnd(data.revenue.total)} /><Kpi label="So với kỳ trước" value={pct(data.revenue.previousChange)} hint={`Kỳ trước ${formatVnd(data.revenue.previous)}`} tone={data.revenue.previousChange < 0 ? "red" : "gold"} /><Kpi label="So với cùng kỳ năm trước" value={pct(data.revenue.yoyChange)} hint={`Cùng kỳ ${formatVnd(data.revenue.yoy)}`} tone={data.revenue.yoyChange < 0 ? "red" : "gold"} /><Kpi label="Đơn đã thu tiền" value={data.revenue.paidOrderCount} /></div>
          ...
          {tab === "orders" && <>
            <div className="grid gap-x-6 sm:grid-cols-2 lg:grid-cols-4"><Kpi label="Tổng đơn đặt" value={data.orders.total} /><Kpi label="Giá trị đơn TB (AOV)" value={formatVnd(data.orders.aov)} /><Kpi label="Tỷ lệ hủy / hoàn" value={pct(data.orders.cancellationRate)} tone="red" /><Kpi label="Tỷ lệ hoàn trả" value={pct(data.orders.returnRate)} tone="red" /></div>
          ...
          {tab === "inventory" && <>
            <div className="grid gap-x-6 sm:grid-cols-2 lg:grid-cols-4"><Kpi label="Giá trị tồn theo giá vốn" value={formatVnd(data.inventory.inventoryValue)} /><Kpi label="Vòng quay tồn kho" value={data.inventory.turnover == null ? "Chưa đủ dữ liệu" : `${data.inventory.turnover.toFixed(2)} vòng`} /><Kpi label="SKU sắp hết" value={data.inventory.lowStock} tone="red" /><Kpi label="Độ phủ giá vốn" value={pct(data.inventory.costCoverage)} hint="Tỷ lệ sản phẩm bán ra đã có giá vốn" /></div>
