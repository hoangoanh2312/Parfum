...
const emptyVoucher = () => ({ code: "", name: "", type: "percentage", value: "10", minOrderValue: "0", maxDiscountAmount: "0", startDate: localDateTime(), endDate: future(30), usageLimit: "0", usageLimitPerUser: "1", stackable: true, userSegment: "ALL", appliesToNewMembers: false, isPrivate: false, isActive: true });
...
function PriceHistoryCard({
  ...
  const percent = previous?.basePrice
    ? (delta / previous.basePrice) * 100
    : 0;
    ...
    <Card className="rounded-lg p-5 shadow-none transition hover:border-gray-300">
          ...
          return (
            <li key={row._id} className="relative">
              ...
              <p className="text-xs text-gray-500">
                ...
                {row.validTo ? formatDate(row.validTo) : "nay"}
              </p>
      ...
      {group.rows.length > 3 && (
        ...
        >
          {expanded
            ? "Thu gọn"
            : `Xem thêm ${group.rows.length - 3} lần điều chỉnh`}
        </button>
      )}
    </Card>
  );
...
export default function AdminPromotions() {
  ...
  const [historySort, setHistorySort] = useState<"recent" | "change" | "name">(
    "recent",
  );
  ...
  const visibleHistoryGroups = useMemo(() => {
    ...
    const filtered = term
      ? historyGroups.filter(
          (group) =>
            group.product.toLowerCase().includes(term) ||
            group.detail.toLowerCase().includes(term),
        )
      : historyGroups;
    const list = [...filtered];
    ...
    if (historySort === "name") {
      ...
      list.sort(
        (left, right) =>
          new Date(right.rows[0].validFrom).getTime() -
          new Date(left.rows[0].validFrom).getTime(),
      );
    }
  ...
  async function save() {
    try {
      ...
      for (const key of ["value", "minOrderValue", "maxDiscountAmount", "usageLimit", "usageLimitPerUser", "priority", "flashPrice", "stockAllocated", "maxPerUser"]) {
      ...
      toast.error(apiMessage(error, error instanceof Error ? error.message : "Không lưu được ưu đãi"));
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Card className="rounded-md p-4 shadow-none">
            ...
            <p className="mt-1 text-2xl font-semibold">
              {historyStats.totalVariants}
            </p>
          </Card>
          <Card className="rounded-md p-4 shadow-none">
            <p className="text-xs text-gray-500">Tổng số lần điều chỉnh</p>
            <p className="mt-1 text-2xl font-semibold">
              {historyStats.totalAdjustments}
            </p>
          </Card>
          <Card className="rounded-md p-4 shadow-none">
            <p className="text-xs text-gray-500">
              Tăng giá gần nhất, mạnh nhất
            </p>
        ...
        <Card className="flex flex-col gap-3 rounded-md p-4 shadow-none lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <Select
              ...
              onChange={(event) =>
                setHistorySort(
                  event.target.value as "recent" | "change" | "name",
                )
              }
              className="sm:w-48"
            >
    ...
    <Modal open={modal} onClose={() => setModal(false)} wide title={editing ? "Chỉnh sửa ưu đãi" : "Tạo ưu đãi"} footer={<><Button variant="secondary" onClick={() => setModal(false)}>Hủy</Button><Button onClick={save} disabled={saving}>{saving ? "Đang lưu..." : "Lưu"}</Button></>}>
      {tab === "vouchers" && (
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Mã voucher" hint="Khách sẽ nhập mã này ở bước thanh toán (tự động viết hoa)">
            <Input value={voucherForm.code} onChange={(e) => setVoucherForm({ ...voucherForm, code: e.target.value.toUpperCase() })} />
          </Field>
          <Field label="Tên chương trình" hint="Tên nội bộ để nhận biết, không hiển thị cho khách">
            <Input value={voucherForm.name} onChange={(e) => setVoucherForm({ ...voucherForm, name: e.target.value })} />
          </Field>
          <Field label="Loại">
          ...
          <Field
            label="Giá trị"
            hint={voucherForm.type === "percentage" ? "Nhập % (ví dụ 10 = giảm 10%). Voucher % tối đa 50% nếu không phải KM tập trung." : voucherForm.type === "fixed" ? "Nhập số tiền giảm (đồng)" : "Miễn phí ship - không cần nhập giá trị"}
          >
            <Input type="number" disabled={voucherForm.type === "free_shipping"} value={voucherForm.value} onChange={(e) => setVoucherForm({ ...voucherForm, value: e.target.value })} />
          </Field>
          <Field label="Đơn tối thiểu" hint="Giá trị đơn hàng tối thiểu để dùng mã (0 = không yêu cầu)">
            <Input type="number" value={voucherForm.minOrderValue} onChange={(e) => setVoucherForm({ ...voucherForm, minOrderValue: e.target.value })} />
          </Field>
          <Field label="Giảm tối đa" hint="Trần số tiền được giảm với voucher %; 0 = không giới hạn">
            <Input type="number" value={voucherForm.maxDiscountAmount} onChange={(e) => setVoucherForm({ ...voucherForm, maxDiscountAmount: e.target.value })} />
          </Field>
          <Field label="Bắt đầu">
            <Input type="datetime-local" value={voucherForm.startDate} onChange={(e) => setVoucherForm({ ...voucherForm, startDate: e.target.value })} />
          </Field>
          <Field label="Kết thúc">
            <Input type="datetime-local" value={voucherForm.endDate} onChange={(e) => setVoucherForm({ ...voucherForm, endDate: e.target.value })} />
          </Field>
          <Field label="Giới hạn toàn hệ thống" hint="Tổng số lượt dùng tối đa; 0 = không giới hạn">
            <Input type="number" value={voucherForm.usageLimit} onChange={(e) => setVoucherForm({ ...voucherForm, usageLimit: e.target.value })} />
          </Field>
          <Field label="Giới hạn mỗi khách" hint="Số lần mỗi khách được dùng; 0 = không giới hạn">
            <Input type="number" value={voucherForm.usageLimitPerUser} onChange={(e) => setVoucherForm({ ...voucherForm, usageLimitPerUser: e.target.value })} />
          </Field>
          <Field label="Phân khúc khách" hint="Mới: chỉ khách chưa có đơn. Tất cả: áp dụng cho cả khách mới và khách đã có đơn.">
            <Select value={voucherForm.userSegment === "NEW" ? "NEW" : "ALL"} onChange={(e) => setVoucherForm({ ...voucherForm, userSegment: e.target.value })}>
          ...
          <div className="space-y-2 pt-6 sm:col-span-2">
            <label className="flex gap-2 text-sm">
              <input type="checkbox" checked={voucherForm.stackable} onChange={(e) => setVoucherForm({ ...voucherForm, stackable: e.target.checked })} />
            ...
            <label className="flex items-start gap-2 text-sm">
              ...
              <span>
                ...
                <span className="block text-xs text-gray-500">Tài khoản khách hàng chưa có đơn sẽ nhận mã khi cập nhật hồ sơ.</span>
              </span>
            </label>
            <label className="flex gap-2 text-sm">
              <input type="checkbox" checked={voucherForm.isActive} onChange={(e) => setVoucherForm({ ...voucherForm, isActive: e.target.checked })} />
              Kích hoạt
            </label>
          </div>
        </div>
