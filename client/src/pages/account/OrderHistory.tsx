      ...
      <main className="px-6 py-10 lg:px-12">
        {loading ? (
          ...
          <div className="space-y-5">
            {orders.map((order) => (
              <article key={order.id} className="border border-[#E2DBD2] bg-[#FFFDF9] p-6">
                <div className="flex flex-col justify-between gap-4 border-b border-[#EAE4DC] pb-5 md:flex-row md:items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="font-serif text-xl">Đơn hàng #{order.id.slice(-6).toUpperCase()}</h2>
                      <p className="mt-1 text-[10px] uppercase tracking-[0.14em] text-[#8A8178]">
                        {order.createdAt ? new Date(order.createdAt).toLocaleDateString("vi-VN") : "Đang cập nhật"}
                        {" · "}
                ...
                <div className="mt-5 space-y-3">
                  {order.items.map((item, index) => (
                    <div key={`${order.id}-${index}`} className="flex justify-between gap-4 text-sm">
                      <span className="text-[#3A342E]">
                        {item.name} × {item.quantity}
                      </span>
                      <span className="text-[#756E67]">{vnd(item.lineTotal)}</span>
                    </div>
                  ))}
                </div>
              </article>
